...
exports.signup = async function (req, res) {
  ...
  const isValiedEmail = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i;
  if (!isValiedEmail.test(email)) {
    return res.send({
      isSuccess: false,
      code: 400,
  ...
  const isValiedPassword = /^(?=.*\d)(?=.*[a-zA-Z])[0-9a-zA-Z]{8,20}$/; //  8 ~ 20자 영문, 숫자 조합
  if (!isValiedPassword.test(password)) {
    return res.send({
      isSuccess: false,
      code: 400,
